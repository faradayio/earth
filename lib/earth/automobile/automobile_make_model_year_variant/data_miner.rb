AutomobileMakeModelYearVariant.class_eval do
  ...
  class AutomobileMakeModelYearVariant::Guru
    ...
    %w{ alpina bentley marquis maybach milan scion smart }.each do |model|
  ...
  class AutomobileMakeModelYearVariant::ParserD
    ...
    def apply(row)
      row.merge!({
        'make'                            => row['Division'],       # match column name with errata
        'model'                           => row['Carline'].upcase, # match column name with errata
        'drive'                           => row['Drive Sys'] + 'WD',
        ...
        'alt_raw_fuel_efficiency_city'    => row['City2 Unadj Fuel FE - Alternative Fuel'],
        'alt_raw_fuel_efficiency_highway' => row['Hwy2 Unadj Fuel FE - Alternative Fuel']
      })
      row
    end
  end
  ...
  data_miner do
    ...
    #     store 'cylinders',  :field_name => 'no_cyl'
    #     store 'displacement'
    #     store 'turbo'
    #     store 'supercharger'
    #     store 'injection'
    ...
    # 2010--?
    {
      ...
      import "#{ year } Fuel Economy Guide",
        ...
        store 'year'
        store 'transmission'
        store 'speeds', :synthesize => proc { |row| row['# Gears'].to_i }
        store 'drive'
        store 'fuel_code'
        ...
        store 'alt_fuel_code'
        store 'alt_raw_fuel_efficiency_city',    :nullify => true, :from_units => :miles_per_gallon, :to_units => :kilometres_per_litre
        store 'alt_raw_fuel_efficiency_highway', :nullify => true, :from_units => :miles_per_gallon, :to_units => :kilometres_per_litre
        store 'cylinders',    :field_name => '# Cyl'
    ...
    process "Calculate adjusted fuel efficiency using the latest EPA equations from EPA Fuel Economy Trends report Appendix A including conversion from miles per gallon to kilometres per litre" do
      ...
      where("alt_raw_fuel_efficiency_city IS NOT NULL").update_all(%{
        alt_fuel_efficiency_city = 1.0 / ((0.003259 / 0.425143707) + (1.1805 / alt_raw_fuel_efficiency_city)),
        alt_fuel_efficiency_city_units = 'kilometres_per_litre'
      })
      where("alt_raw_fuel_efficiency_highway IS NOT NULL").update_all(%{
        alt_fuel_efficiency_highway = 1.0 / ((0.001376 / 0.425143707) + (1.3466 / alt_raw_fuel_efficiency_highway)),
