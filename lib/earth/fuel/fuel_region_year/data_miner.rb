FuelRegionYear.class_eval do
  data_miner do
    schema Earth.database_options do
      string  'name'
      string  'fuel_region_name'
      string  'fuel_name'
      string  'region_name'
      integer 'year'
    ...
    process "Calculate CO2 and CO2 biogenic emission factors" do
      ...
      FuelRegionYear.all.each do |record|
    ...
    verify "Fuel region name should never be missing" do
      FuelRegionYear.all.each do |record|
        fuel_region_name = record.send(:fuel_region_name)
        if fuel_region_name.nil?
          raise "Missing fuel region name for FuelRegionYear '#{record.name}'"
        end
      end
    end
    
    # FIXME TODO verify fuel_name is in Fuel
    # FIXME TODO verify region_name is in Region
    
    verify "Year should be from 1990 to 2008" do
      FuelRegionYear.all.each do |record|
        year = record.send(:year)
        unless year > 1989 and year < 2009
          raise "Invalid year for FuelRegionYear '#{record.name}': #{year} (should be from 1990 to 2008)"
        end
      end
    end
    ...
    verify "Carbon content and energy content should be greater than zero" do
      FuelRegionYear.all.each do |record|
        %w{ carbon_content energy_content }.each do |attribute|
          ...
          unless value > 0
            raise "Invalid #{attribute} for FuelRegionYear '#{record.name}': #{value} (should be > 0)"
          end
        end
      end
    end
